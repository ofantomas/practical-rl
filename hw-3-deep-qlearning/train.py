...
def train(env, agent, target_network, exp_replay, loss_func, device, lr=1e-4, 
          total_steps=3 * 10**6, verbose_steps=3 * 10 ** 5, batch_size=32, h=64, w=64,
    ...
    state = env.reset()
    with tqdm(range(total_steps + 1), total=total_steps + 1) as iterator:
        for step in iterator:
            ...
            if (step % eval_freq == 0):
                # eval the agent
                current_rw = utils.evaluate(make_env(seed=step, h=h, w=w), agent,
                                            n_games=3, greedy=True, t_max=1000)
                iterator.set_description("Mean reward: {:3.1f}".format(current_rw))
                mean_rw_history.append(current_rw)
                initial_state_q_values = agent.get_qvalues(
                    [make_env(seed=step, h=h, w=w).reset()]
                )
...
def train_PER(env, agent, target_network, PER, loss_func, device, lr=1e-4, 
          total_steps=3 * 10**6, verbose_steps=3 * 10 ** 5, batch_size=32, h=64, w=64,
          decay_steps=1 * 10**6, init_epsilon=1.0, final_epsilon=0.1, timesteps_per_epoch=1,
          max_grad_norm=50, loss_freq=50, refresh_target_network_freq=5000, eval_freq=5000):
    stop_evaluation = False
    ...
    with tqdm(range(total_steps + 1), total=total_steps + 1) as iterator:
        for step in iterator:
            ...
            # play
            _, state = utils.play_and_record_PER(state, agent, env, PER, timesteps_per_epoch)

            # train
            states, actions, rewards, next_states, is_done, weights, indices = PER.sample(batch_size)
            loss, errors = loss_func(states, actions, rewards, next_states, is_done, weights,
                                     agent, target_network, device)

            loss.backward()
            PER.sampler.update_priorities(indices, errors)
            grad_norm = nn.utils.clip_grad_norm_(agent.parameters(), max_grad_norm)
            opt.step()
            opt.zero_grad()
